...
class BuscaViewController: UIViewController {
    ...
    Filme(imagem: UIImage(named: "sextaFeiraImagem"), nome: "Sexta Feira 13", ano: 1980, descricao: "Um assassiono mata dois jovens em um acampamento. Após reaberto, monitores começam a desaparecer mortos por um maníaco", maiorDeIdade: true),
    ...
    Filme(imagem: UIImage(named: "aOrfaImagem"), nome: "A Orfã", ano: 2009, descricao: "Um casal adota uma criança. Estranhos acontecimentos fazem parte do histórico da menina", maiorDeIdade: true),
    ...
    Filme(imagem: UIImage(named: "doceVingancaImagem"), nome: "Doce Vingança", ano: 2010, descricao: "Uma jovem escritora planeja sua vingança após ser atacada por moradores locais ", maiorDeIdade: true),
    Filme(imagem: UIImage(named: "massacreDaSerraEletricaImagem"), nome: "O Massacre da Serra Elétrica", ano: 1974, descricao: "Um grupo de amigos é atacado por um homem de máscara após um terrível massacre", maiorDeIdade: true),
    ...
    Filme(imagem: UIImage(named: "escolhaOuMorraImagem"), nome: "Escolha ou Morra", ano: 2022, descricao: "Uma estudante falida entra em uma jogo obscuro de computador", maiorDeIdade: true),
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        nomeFilmeTextField.delegate = self
    ...
    @IBAction func buscarNome(_ sender: Any) {
        
        let resultados = addFilmes.filter({ filme in
            return filme.nome.contains(nomeFilmeTextField.text?.lowercased() ?? "")
        })
        
        if nomeFilmeTextField.hasText {
            addFilmes = resultados
        } else {
            addFilmes = []
        }
        buscaTableView.reloadData()
    }
